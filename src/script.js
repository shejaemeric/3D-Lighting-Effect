...
const pointLight = new THREE.PointLight("yellow", 1, 10, 2);
...
const hemisphereLight = new THREE.HemisphereLight("black", 0x4e00ff, 0.3);
// scene.add(hemisphereLight);

const rectLight = new THREE.RectAreaLight(0x4e00ff, 2, 4, 1);
...
gui
  ...
  .name("Rect Light X position")
  .min(-10)
  .max(10)
  .step(0.001);
gui
  .add(rectLight.position, "y")
  .name("Rect Light Y position")
  .min(-10)
  .max(10)
  .step(0.001);
gui
  .add(rectLight.position, "z")
  .name("Rect Light Z position")
  .min(-10)
  .max(10)
  .step(0.001);
...
//   let R = Math.round((e.pageX / sizes.width) * 255);
//   let G = Math.round((e.pageY / sizes.height) * 255);
//   let B = 150;
//   rectLight.color.set(
//     new THREE.Color((e.pageX * 5) % 255, (e.pageY * 5) % 255, 15)
//   );
...
const tick = () => {
  ...
  //       rectLight.color = new THREE.Color(
  //         Math.floor((Math.random() * elapsedTime) % 256),
  //         Math.floor((Math.random() * elapsedTime) % 256),
  //         Math.floor((Math.random() * elapsedTime) % 256)
  //       );
  //     }
